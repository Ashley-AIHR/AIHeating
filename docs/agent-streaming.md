...
Low reasoning effort is requested for tool-selection rounds with a 3,500-token completion budget. The reserved final-answer round explicitly disables reasoning: hiding reasoning alone does not stop it consuming the output budget. A deployed diagnostic exposed this distinction by completing its tools but returning no public text. Completion finish reasons and reasoning-token counts are retained in activity metadata, without retaining private reasoning content. These settings reduce latency but cannot guarantee provider availability or a completed explanation. Provider usage after an interrupted stream may be unavailable and must not be inferred as zero cost.
