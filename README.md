...
The default route is an authored winter-city vision inspired by Yinchuan: detailed modular residential architecture, landscaped streets and a cutaway blue-and-steel energy centre, all orbitable within one connected workspace. B01–B12 are coherent simulated thermal loads. The geometry is design imagery made into real interactive geometry, not a surveyed estate. The public 926-component BIM can be associated with individual simulated assets as reference evidence, not a verified field mapping or the visual goal. A read-only authenticated observation gateway accepts validated measurements but does not assimilate them into the simulator.

The standalone engineering tools are preserved at **/engineering** (including the independent OpenDHN benchmark), image-backed reference design at **/reference**, earlier procedural scene at **/operations-classic**, and frozen research dashboard at **/legacy**. The integrated workspace supports fictional Yinchuan, Beijing and Shanghai scenarios with distinct visual identities and synthetic model inputs. No physical plant control is connected.

## Fullscreen operations and item-level BIM

Use **Expand to full screen** in the contextual panel for Assets, Alarms or Agent Operations (also available in other panels). **Restore panel** or Escape returns to the district. Selection and mission state remain mounted; keyboard focus stays within the expanded panel, and opening an item BIM dialog does not lose the underlying workspace.

Select a building, branch or station device, then use its **BIM · item ID** viewport button or **Open item BIM** action. Choose a source component, add review observations and explicitly **Link component to this item**. This creates a session-only reference association scoped to city, simulation context, asset and optional equipment ID. Use **Update linked component and evidence** after changes. Switching items does not transfer notes; linking another component does not establish a hydraulic or sensor binding. Linked public-source reviews are restored on returning to an item; local GLB geometry must be re-imported after changing items. Unlinked drafts are not saved across item changes.

The item's linked review accompanies both BIM-originated investigations and normal agent missions. The server rejects references belonging to a different city, simulation context, asset or selected mechanical device. Removing the link in Assets stops its inclusion in future missions. Links are not a persistent asset-management database and do not survive a browser reload or simulation reset.
