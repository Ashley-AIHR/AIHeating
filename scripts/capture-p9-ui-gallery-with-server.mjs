...
try {
  ...
  const capture = spawn(process.execPath, ['scripts/capture-p9-ui-gallery.mjs'], { cwd: process.cwd(), stdio: 'inherit' });
  const code = await new Promise((resolve) => capture.on('exit', resolve));
